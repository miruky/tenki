...
ヘッダーの星印で表示中の地域をお気に入りに登録すると、ツールバー下にチップが並び、ワンタップで切り替えられる(最大12件)。配色は端末設定に従いつつ手動でも切り替えられ、リロード時のちらつきは描画前のスクリプトで抑える。`r` で更新、`t` でテーマ切替、`f` でお気に入り登録のキーボード操作に対応し、タブへ戻った際に前回取得から30分以上経っていれば静かに再取得する。
...
- `src/lib/favorites.ts` お気に入り地域の保存・検証
